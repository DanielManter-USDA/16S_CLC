...
subtitle:  | 
    | "Manter et al. 2024. Unveiling hidden errors in soil microbial community 
    |  sequencing: A case for reference soils and improved diagnostics. 
...
Oxford Nanopore minION in different laboratories. Code provided is for all data 
...
```
\*denotes a required input file/folder


## Load required packages
```{r setup, include=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
knitr::opts_knit$set(root.dir = 'D:\\CLC_16S-main')
...
ggsave(p, file='figures/Figure.1b.png', dpi=300, height=3.5, width=7.25, units='in')
...
ggsave(p, file='figures/Figure.1C_left.png', dpi=300, height=3.5, width=3.75, units='in')
...
p
p <- set_panel_size(p, height=unit(2, "in"), width=unit(2, "in"))
ggsave(p, file='figures/Figure.1C_right.png', dpi=300, height=3.5, width=3.75, units='in')
```
...
# rename taxa according to new nomenclature
tax_table(P.10)[,'phylum'] <- gsub("Acidobacteria", "Acidobacteriota", tax_table(P.10)[,'phylum'])
tax_table(P.10)[,'phylum'] <- gsub("Actinobacteria", "Actinomycetota", tax_table(P.10)[,'phylum'])
tax_table(P.10)[,'phylum'] <- gsub("Bacteroidetes", "Bacteroidota", tax_table(P.10)[,'phylum'])
tax_table(P.10)[,'phylum'] <- gsub("Chloroflexi", "Chloroflexota", tax_table(P.10)[,'phylum'])
tax_table(P.10)[,'phylum'] <- gsub("Firmicutes", "Bacillota", tax_table(P.10)[,'phylum'])
tax_table(P.10)[,'phylum'] <- gsub("Gemmatimonadetes", "Gemmatimonadota", tax_table(P.10)[,'phylum'])
tax_table(P.10)[,'phylum'] <- gsub("Planctomycetes", "Planctomycetota", tax_table(P.10)[,'phylum'])
tax_table(P.10)[,'phylum'] <- gsub("Proteobacteria", "Pseudomonadota", tax_table(P.10)[,'phylum'])
tax_table(P.10)[,'phylum'] <- gsub("Verrucomicrobia", "Verrucomicrobiota", tax_table(P.10)[,'phylum'])
...
p <- ggplot(d, aes(x=Code.name, y=Abundance, fill=phylum)) +
  ...
  theme(text=element_text(size=10,  family="sans"))
p
p <- set_panel_size(p, height=unit(1.25, "in"), width=unit(1.4, "in"))
ggsave(p, file='figures/Figure.2a.png', dpi=300, height=4, width=7.25, units='in')
...
tax_table(P.soils)[,'Phylum'] <- gsub("Actinobacteria", "Actinomycetota", tax_table(P.soils)[,'Phylum'])
tax_table(P.soils)[,'Phylum'] <- gsub("Bacteroidetes", "Bacteroidota", tax_table(P.soils)[,'Phylum'])
...
d <- readRDS('emu_tree/EMU.phylip.dist.RDS') # create distance matrix if necessary with "CLC_Analysis.NJ_Tree.Rmd"

# check if any taxa are in phyloseq object but not tree - these will be removed in the next few steps
taxa_names(P.glom)[!taxa_names(P.glom) %in% names(d)]
...
p <- ggtree(P.merge, color='grey50') +
    ...
    geom_tiplab(aes(label=Family), size=2, align=TRUE, offset=0.05, hjust=-0.01, family='serif') +
...
# calculate Log2FC between each lab and the ave of the primary labs
final <- melt_all %>%
...
q <- p + new_scale_fill() + new_scale_color() +
         geom_fruit(data=final[final$Library=='Seq',], geom=geom_tile,
                  mapping=aes(y=OTU, x=lname, fill=val),
                  offset=0.8, pwidth = 0.5, color='black', 
                  linewidth=0.1,
                  axis.params=list(
                         axis       = "x",
                         text.size  = 2,
         ...
         geom_fruit(data=final[final$Library=='PCR/Seq',], geom=geom_tile,
                  mapping=aes(y=OTU, x=lname, fill=val),
                  offset=0.1, pwidth = 0.5, color='black',
                  linewidth=0.1,
                  axis.params=list(
                         axis       = "x",
                         text.size  = 2,
         ...
         geom_fruit(data=final[final$Library=='Ext/PCR/Seq',], geom=geom_tile,
                  ...
                  axis.params=list(
                         ...
                         hjust = 1,
                         vjust = 0.5)) +
         scale_fill_viridis(direction=-1, option = "G", na.value='grey75', name='Log2 CPM') +
         scale_color_gradient(low='white', high=muted('green'),
                              na.value='grey75', name='Log2 CPM') +
...
#P.soils <- subset_samples(P.soils, Code.name != 'Lab2.a')
...
means <- final %>%
  ...
  dplyr::summarize(Range = diff(range(log2FC, na.rm=FALSE)),
...
p <- plot(emm, comparisons=T, horizontal=FALSE, adjust='fdr', CIs=TRUE) +
...
emm <- emmeans::emmeans(res, "Library", data=means)
pairs(emm, adjust='fdr')
p <- plot(emm, comparisons=T, horizontal=FALSE, adjust='fdr') +
  labs(x='Rel. Abund. Variability', y='') +
  xlim(0,6) +
  theme_bw() +
  theme(axis.text.x=element_text(angle=45, hjust=1)) +
...
# rename taxa according to new nomenclature
final$Phylum <- gsub("Acidobacteria", "Acidobacteriota", final$Phylum)
final$Phylum <- gsub("Actinobacteria", "Actinomycetota", final$Phylum)
final$Phylum <- gsub("Aquificae", "Aquificota", final$Phylum)
final$Phylum <- gsub("Armatimonadetes", "Armatimonadota", final$Phylum)
final$Phylum <- gsub("Bacteroidetes", "Bacteroidota", final$Phylum)
final$Phylum <- gsub("Calditrichaeota", "Calditrichota", final$Phylum)
final$Phylum <- gsub("Chloroflexi", "Chloroflexota", final$Phylum)
final$Phylum <- gsub("Chlorobi", "Chlorobiota", final$Phylum)
final$Phylum <- gsub("Deferribacteres", "Deferribacterota", final$Phylum)
final$Phylum <- gsub("Deinococcus-Thermus", "Deinococcota", final$Phylum)
final$Phylum <- gsub("Elusimicrobia", "Elusimicrobiota", final$Phylum)
final$Phylum <- gsub("Fibrobacteres", "Fibrobacterota", final$Phylum)
final$Phylum <- gsub("Firmicutes", "Bacillota", final$Phylum)
final$Phylum <- gsub("Gemmatimonadetes", "Gemmatimonadota", final$Phylum)
final$Phylum <- gsub("Ignavibacteriae", "Ignavibacteriota", final$Phylum)
final$Phylum <- gsub("Nitrospinae", "Nitrospinota", final$Phylum)
final$Phylum <- gsub("Nitrospirae", "Nitrospirota", final$Phylum)
final$Phylum <- gsub("Planctomycetes", "Planctomycetota", final$Phylum)
final$Phylum <- gsub("Proteobacteria", "Pseudomonadota", final$Phylum)
final$Phylum <- gsub("Rhodothermaeota", "Rhodothermota", final$Phylum)
final$Phylum <- gsub("Spirochaetes", "Spirochaetota", final$Phylum)
final$Phylum <- gsub("Tenericutes", "Mycoplasmatota", final$Phylum)
final$Phylum <- gsub("Thermodesulfobacteria", "Thermodesulfobacteriota", final$Phylum)
final$Phylum <- gsub("Thermotogae", "Thermotogota", final$Phylum)
final$Phylum <- gsub("Verrucomicrobia", "Verrucomicrobiota", final$Phylum)
...
means <- MAD %>%
  ...
  dplyr::summarize(Range = diff(range(ef_logFC, na.rm=TRUE)),
                   mean = mean(mean, na.rm=TRUE),
                   n = n())
...
ggsave(p, file='figures/Ext_Data_Figure.1.png', dpi=300, height=4, width=5.25, units='in')
```
...
ggsave(p, file='figures/Ext_Data_Figure.2a.png', dpi=300, height=3.1, width=7, units='in')
...
p
p <- set_panel_size(p, height=unit(1.5, "in"), width=unit(1.75, "in"))
ggsave(p, file='figures/Ext_Data_Figure.2b_top.png', dpi=300, height=3.1, width=7, units='in')
...
p
p <- set_panel_size(p, height=unit(1.5, "in"), width=unit(1.75, "in"))
ggsave(p, file='figures/Ext_Data_Figure.2b_bottom.png', dpi=300, height=3.1, width=7, units='in')
...
p
p <- set_panel_size(p, height=unit(1.5, "in"), width=unit(1.75, "in"))
ggsave(p, file='figures/Ext_Data_Figure.3a.png', dpi=300, height=3.1, width=7, units='in')
...
p
p <- set_panel_size(p, height=unit(1.5, "in"), width=unit(1.75, "in"))
ggsave(p, file='figures/Ext_Data_Figure.3b_top.png', dpi=300, height=3.1, width=7, units='in')
...
p
p <- set_panel_size(p, height=unit(1.5, "in"), width=unit(1.75, "in"))
ggsave(p, file='figures/Ext_Data_Figure.3b_bottom.png', dpi=300, height=3.1, width=7, units='in')

```


## Reported in Text only - Comparison of thermal cyclers in Lab2.b sequencing run
```{r}
# import phyloseq object created above
P.count <- readRDS('P.count.RDS')

P.sub <- subset_samples(P.count, Code.name == 'Lab2.b' & Plot %in% c('86E', 'Plot3') 
                        & Rep %in% c('RepA', 'RepB') & Library == 'PCR/Seq')
